    ...
    */
    RGraph.HBar.prototype.Draw = function ()
    {
        ...
        */
        this.graphwidth     = RGraph.GetWidth(this) - ( (4 * gutter));
        this.graphheight    = RGraph.GetHeight(this) - (2 * gutter);
                        ...
                        */
                        if (obj.Get('chart.tooltips.event') == 'onmousemove') {
                            ...
                            */
                            ...
                            if (text) {
                                if (!tooltipObj || tooltipObj.__index__ != i) {
    ...
    */
    ...
    {
        var gutter  = this.Get('chart.gutter');
        var halfway = ((RGraph.GetWidth(this) - (4 * gutter)) / 2) + (3 * gutter);
        ...
        if (this.Get('chart.yaxispos') == 'center') {
            ...
            this.context.lineTo(halfway, RGraph.GetHeight(this) - gutter);
        } else {
        ...
        // Draw the X axis
        this.context.moveTo(gutter * 3, RGraph.GetHeight(this) - gutter);
        this.context.lineTo(RGraph.GetWidth(this) - gutter, RGraph.GetHeight(this) - gutter);

        // Draw the Y tickmarks
        var yTickGap = (RGraph.GetHeight(this) - (2 * gutter)) / this.data.length;

        for (y=gutter; y<(RGraph.GetHeight(this) - gutter); y+=yTickGap) {
            if (this.Get('chart.yaxispos') == 'center') {
        ...
        xTickGap = (RGraph.GetWidth(this) - (4 * gutter) ) / 10;
        yStart   = RGraph.GetHeight(this) - gutter;
        yEnd     = (RGraph.GetHeight(this) - gutter) + 3;

        for (x=(RGraph.GetWidth(this) - gutter), i=0; this.Get('chart.yaxispos') == 'center' ? x>=(3 * gutter) : x>(3*gutter); x-=xTickGap) {
    ...
    */
    RGraph.HBar.prototype.DrawBackground = function ()
    {
        ...
        for (var i=gutter; i < (RGraph.GetHeight(this) - gutter); i+=80) {
            context.fillRect (gutter * 3, i, RGraph.GetWidth(this) - (gutter * 4), Math.min(40, RGraph.GetHeight(this) - gutter - i) );
        }

        this.context.fillStyle = this.Get('chart.background.barcolor2');
        for (var i= (40 + gutter); i < (RGraph.GetHeight(this) - gutter); i+=80) {
            context.fillRect (gutter * 3, i, RGraph.GetWidth(this) - (gutter * 4), i + 40 > (RGraph.GetHeight(this) - gutter) ? RGraph.GetHeight(this) - (gutter + i) : 40);
        }
        ...
        if (this.Get('chart.background.grid')) {
            ...
            if (this.Get('chart.background.grid.autofit')) {
                var vsize = (RGraph.GetWidth(this) - (4 * gutter)) / this.Get('chart.background.grid.autofit.numvlines');
                var hsize = (RGraph.GetHeight(this) - (4 * gutter)) / this.Get('chart.background.grid.autofit.numhlines');
            ...
            if (this.Get('chart.background.grid.hlines')) {
                for (y=gutter; y < (RGraph.GetHeight(this) - gutter); y+=this.Get('chart.background.grid.hsize')) {
                    context.moveTo(gutter * 3, y);
                    context.lineTo(RGraph.GetWidth(this) - gutter, y);
                }
            }

            // Draw the vertical lines
            if (this.Get('chart.background.grid.vlines')) {
                for (x=gutter * 3; x <= (RGraph.GetWidth(this) - gutter); x+=this.Get('chart.background.grid.vsize')) {
                    context.moveTo(x, gutter);
                    context.lineTo(x, RGraph.GetHeight(this) - gutter);
                }
            }
            ...
            if (this.Get('chart.background.grid.border')) {
                ...
                context.strokeStyle = this.Get('chart.background.grid.color');
                context.strokeRect(gutter * 3, gutter, RGraph.GetWidth(this) - (4 * gutter), RGraph.GetHeight(this) - (2 * gutter));
            }
        }
        ...
        if (typeof(this.Get('chart.title.xaxis')) == 'string' && this.Get('chart.title.xaxis').length) {        
            context.beginPath();
            RGraph.Text(context, font, size + 2, RGraph.GetWidth(this) / 2, RGraph.GetHeight(this) - (gutter * this.Get('chart.title.xaxis.pos')), this.Get('chart.title.xaxis'), 'center', 'center', false, false, false, true);
            context.fill();
        }
        ...
        if (typeof(this.Get('chart.title.yaxis')) == 'string' && this.Get('chart.title.yaxis').length) {
            context.beginPath();
            RGraph.Text(context, font, size + 2, gutter * this.Get('chart.title.yaxis.pos'), RGraph.GetHeight(this) / 2, this.Get('chart.title.yaxis'), 'center', 'center', false, 270, false, true);
            context.fill();
        }
    }
    ...
    RGraph.HBar.prototype.DrawLabels = function ()
    {
        ...
        */
        if (this.Get('chart.xlabels')) {
            ...
            if (this.Get('chart.yaxispos') == 'center') {
                RGraph.Text(context, font, text_size, (gutter * 3) + (this.graphwidth * (10/10)), gutter + this.halfTextHeight + this.graphheight + 2, RGraph.number_format(this, this.scale[4].toFixed(this.Get('chart.scale.decimals')), units_pre, units_post), 'center', 'center');
                RGraph.Text(context, font, text_size, (gutter * 3) + (this.graphwidth * (9/10)), gutter + this.halfTextHeight + this.graphheight + 2, RGraph.number_format(this, this.scale[3].toFixed(this.Get('chart.scale.decimals')), units_pre, units_post), 'center', 'center');
                RGraph.Text(context, font, text_size, (gutter * 3) + (this.graphwidth * (8/10)), gutter + this.halfTextHeight + this.graphheight + 2, RGraph.number_format(this, this.scale[2].toFixed(this.Get('chart.scale.decimals')), units_pre, units_post), 'center', 'center');
                RGraph.Text(context, font, text_size, (gutter * 3) + (this.graphwidth * (7/10)), gutter + this.halfTextHeight + this.graphheight + 2, RGraph.number_format(this, this.scale[1].toFixed(this.Get('chart.scale.decimals')), units_pre, units_post), 'center', 'center');
                RGraph.Text(context, font, text_size, (gutter * 3) + (this.graphwidth * (6/10)), gutter + this.halfTextHeight + this.graphheight + 2, RGraph.number_format(this, this.scale[0].toFixed(this.Get('chart.scale.decimals')), units_pre, units_post), 'center', 'center');

                RGraph.Text(context, font, text_size, (gutter * 3) + (this.graphwidth * (4/10)), gutter + this.halfTextHeight + this.graphheight + 2, '-' + RGraph.number_format(this, this.scale[0].toFixed(this.Get('chart.scale.decimals')), units_pre, units_post), 'center', 'center');
                RGraph.Text(context, font, text_size, (gutter * 3) + (this.graphwidth * (3/10)), gutter + this.halfTextHeight + this.graphheight + 2, '-' + RGraph.number_format(this, this.scale[1].toFixed(this.Get('chart.scale.decimals')), units_pre, units_post), 'center', 'center');
                RGraph.Text(context, font, text_size, (gutter * 3) + (this.graphwidth * (2/10)), gutter + this.halfTextHeight + this.graphheight + 2, '-' + RGraph.number_format(this, this.scale[2].toFixed(this.Get('chart.scale.decimals')), units_pre, units_post), 'center', 'center');
                RGraph.Text(context, font, text_size, (gutter * 3) + (this.graphwidth * (1/10)), gutter + this.halfTextHeight + this.graphheight + 2, '-' + RGraph.number_format(this, this.scale[3].toFixed(this.Get('chart.scale.decimals')), units_pre, units_post), 'center', 'center');
                RGraph.Text(context, font, text_size, (gutter * 3) + (this.graphwidth * (0)), gutter + this.halfTextHeight + this.graphheight + 2, '-' + RGraph.number_format(this, this.scale[4].toFixed(this.Get('chart.scale.decimals')), units_pre, units_post), 'center', 'center');
    
            } else {
    
                RGraph.Text(context, font, text_size, (gutter * 3) + (this.graphwidth * (5/5)), gutter + this.halfTextHeight + this.graphheight + 2, RGraph.number_format(this, this.scale[4].toFixed(this.Get('chart.scale.decimals')), units_pre, units_post), 'center', 'center');

                RGraph.Text(context, font, text_size, (gutter * 3) + (this.graphwidth * (4/5)), gutter + this.halfTextHeight + this.graphheight + 2, RGraph.number_format(this, this.scale[3].toFixed(this.Get('chart.scale.decimals')), units_pre, units_post), 'center', 'center');
                RGraph.Text(context, font, text_size, (gutter * 3) + (this.graphwidth * (3/5)), gutter + this.halfTextHeight + this.graphheight + 2, RGraph.number_format(this, this.scale[2].toFixed(this.Get('chart.scale.decimals')), units_pre, units_post), 'center', 'center');
                RGraph.Text(context, font, text_size, (gutter * 3) + (this.graphwidth * (2/5)), gutter + this.halfTextHeight + this.graphheight + 2, RGraph.number_format(this, this.scale[1].toFixed(this.Get('chart.scale.decimals')), units_pre, units_post), 'center', 'center');
                RGraph.Text(context, font, text_size, (gutter * 3) + (this.graphwidth * (1/5)), gutter + this.halfTextHeight + this.graphheight + 2, RGraph.number_format(this, this.scale[0].toFixed(this.Get('chart.scale.decimals')), units_pre, units_post), 'center', 'center');
                
                if (this.Get('chart.xmin') > 0) {
                    RGraph.Text(context,font,text_size,(gutter * 3),gutter + this.halfTextHeight + this.graphheight + 2,RGraph.number_format(this, this.Get('chart.xmin'), units_pre, units_post),'center','center');
                }
            }
        ...
        */
        if (typeof(this.Get('chart.labels')) == 'object') {
            ...
            var barHeight = (RGraph.GetHeight(this) - (2 * gutter) ) / this.Get('chart.labels').length;
            
            // Reset the xTickGap
            yTickGap = (RGraph.GetHeight(this) - (2 * gutter)) / this.Get('chart.labels').length

            // Draw the X tickmarks
            var i=0;
            for (y=gutter + (yTickGap / 2); y<=RGraph.GetHeight(this) - gutter; y+=yTickGap) {
                RGraph.Text(this.context, font,this.Get('chart.text.size'),(gutter * 3) - xOffset,y,String(this.Get('chart.labels')[i++]),'center','right');
            }
        }
    }
    ...
    */
    RGraph.HBar.prototype.Drawbars = function ()
    {
        ...
        */
        var gutter     = this.Get('chart.gutter');
        var graphwidth = (RGraph.GetWidth(this) - (4 * gutter));
            ...
            */
            this.context.beginPath();
                if (typeof(this.data[i]) == 'number') {
                    ...
                    this.coords.push([x, y + vmargin, barWidth, height - (2 * vmargin), this.Get('chart.colors')[0], this.data[i]]);
                ...
                */
                    ...
                    for (j=0; j<this.data[i].length; ++j) {
                        ...
                        */
                        ...
                        var width = ((this.data[i][j] - this.Get('chart.xmin')) / (this.max - this.Get('chart.xmin'))) * (RGraph.GetWidth(this) - (4 * gutter) );
